...
library(tidyverse)
library(gt)
library(tidyjson); library(jsonlite)
library(httr)
```
...
(https://ghoapi.azureedge.net/api/MCV2)
...
```{r}
# creating function to extract data from json data
row <- function(v){
  d |> 
    select(v) |> 
    unnest_wider(v) |> 
    select(ParentLocationCode, ParentLocation, SpatialDim, TimeDim, Value, NumericValue)
}
```
